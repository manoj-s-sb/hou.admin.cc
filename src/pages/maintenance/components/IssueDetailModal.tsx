...
const IssueDetailModal = ({ item, index, onClose, onSuccess, dispatch, updatedBy }: IssueDetailModalProps) => {
  ...
  const steps = timelineActs.length > 0
    ? timelineActs.map(a => ({
  ...
  const activityEntries: { text: string; at: string; action: string; byName: string | null; attachmentUrl: string | null }[] =
    acts.length > 0
      ? acts.map((a: WorkActivity) => ({ text: resolveActivityLabel(a), at: a.at, action: a.action, byName: a.byName, attachmentUrl: a.attachmentUrl || null }))
      : [{ text: `Issue raised by ${resolvedRaisedBy}`, at: currentItem.createdAt, action: 'raised', byName: resolvedRaisedBy, attachmentUrl: null }];
  ...
  const handleRemoveCommentAttachment = () => {
    if (commentAttachment) deleteWorkMedia(commentAttachment);
              ...
              <div className="border-t border-gray-200 px-4 py-2">
                {commentAttachment && (
                  <div className="mb-2 flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5">
                    ...
                    <span className="flex-1 truncate text-xs text-gray-600">{commentAttachment.split('/').pop()}</span>
                    <button className="text-xs text-red-400 hover:text-red-600" type="button" onClick={handleRemoveCommentAttachment}>Remove</button>
                  </div>
                )}
