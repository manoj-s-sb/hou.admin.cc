...
import { FACILITY_CODE, Tab, TaskFrequency, tabs, taskFrequencies } from './constants';
...
const Maintenance = () => {
  ...
  const statusRenderCell = (params: any) => {
    ...
    return <span className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${map[s] || 'bg-gray-100 text-gray-600'}`}>{s}</span>;
  };
  ...
  const priorityRenderCell = (params: any) => {
    ...
    return <span className={`rounded-full px-3 py-1 text-xs font-semibold capitalize ${map[p] || 'bg-gray-100 text-gray-600'}`}>{p || '-'}</span>;
  };
  ...
  const columnsMap: Record<Tab, ColumnDef[]> = {
    task: [
      ...
      { field: 'frequency', headerName: 'Frequency', flex: 1, sortable: true, valueGetter: p => p.row?.frequency || '-' },
      { field: 'laneId', headerName: 'Lane', width: 80, sortable: true, valueGetter: p => p.row?.laneId ?? '-' },
    ...
    log: [
      ...
      { field: 'laneId', headerName: 'Lane', flex: 1, sortable: true, valueGetter: p => (p.row?.laneId ? `Lane ${p.row.laneId}` : '-') },
      {
        field: 'frequency',
        headerName: 'Frequency',
        flex: 1,
        sortable: true,
        valueGetter: p => { const f = p.row?.frequency; return f ? f.charAt(0).toUpperCase() + f.slice(1) : '-'; },
      },
      {
        field: 'status',
        headerName: 'Action',
      ...
      { field: 'createdBy', headerName: 'Created By', flex: 1, sortable: true, valueGetter: p => p.row?.raisedByName || p.row?.updatedBy || p.row?.createdBy || '-' },
      {
        ...
        valueGetter: p => {
          ...
          return new Date(p.row.createdAt).toLocaleString('en-US', { day: '2-digit', hour: '2-digit', minute: '2-digit', month: 'short' });
        },
      },
    ],
    schedule: [
      ...
      { field: 'category', headerName: 'Category', flex: 1, sortable: true, valueGetter: p => p.row?.category || '-' },
      { field: 'frequency', headerName: 'Frequency', flex: 1, sortable: true, valueGetter: p => p.row?.frequency || '-' },
      { field: 'scheduledDate', headerName: 'Scheduled Date', flex: 1.2, sortable: true, valueGetter: p => p.row?.scheduledDate || '-' },
      { field: 'laneId', headerName: 'Lane', width: 80, sortable: true, valueGetter: p => p.row?.laneId ?? '-' },
      { field: 'status', headerName: 'Status', flex: 1, sortable: true, renderCell: statusRenderCell },
    ],
  };
              ...
              <div className="flex">
                {Array.from({ length: 7 }, (_, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-sm font-bold ${isSelected ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>
                        {dayLabel}
                      </span>
                ...
                {(() => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-sm font-bold ${isSelected ? 'text-gray-900' : 'text-gray-500 hover:text-gray-700'}`}>Overdue</span>
