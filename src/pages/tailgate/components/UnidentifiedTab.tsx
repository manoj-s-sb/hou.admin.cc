                  ...
                  <tr key={`hdr-${dv}`} className="bg-gray-50/80">
                    <td className="px-4 py-2" colSpan={6}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[11px] text-gray-400">{items.length} event{items.length !== 1 ? 's' : ''}</span>
                      </div>
                    </td>
                  </tr>,
                  ...items.map((row) => {
                    ...
                    const { label: evLabel, className: evCls, gradient } = eventMap[evType] || { label: evType, className: 'bg-gray-100 text-gray-600', gradient: 'linear-gradient(135deg,#1f2937,#4b5563)' };
                      ...
                      <tr key={row.id} className="border-t border-gray-100 bg-yellow-50/30 hover:bg-yellow-50">
                        ...
                        <td className="px-4 py-3">
                          <button className="flex flex-col items-center gap-1" type="button" onClick={() => onVideoClick(row)}>
                            <div className="relative flex h-11 w-16 items-center justify-center overflow-hidden rounded-lg" style={{ background: row.snapshotUrl ? undefined : gradient }}>
                              {row.snapshotUrl && <img alt="snapshot" className="absolute inset-0 h-full w-full object-cover" src={row.snapshotUrl} />}
                              <div className="absolute inset-0 flex items-center justify-center bg-black/25">
                                <div className="flex h-5 w-5 items-center justify-center rounded-full bg-white/90">
                                  <svg className="ml-0.5 h-2.5 w-2.5 text-[#21295A]" fill="currentColor" viewBox="0 0 10 12"><polygon points="1,0 9,6 1,12" /></svg>
                                </div>
                              </div>
                            </div>
                        ...
                        <td className="whitespace-nowrap px-4 py-3">
                          <span className="rounded bg-blue-50 px-2 py-0.5 text-[11px] font-semibold text-blue-700">{row.door?.name ?? '—'}</span>
                        </td>
                        <td className="whitespace-nowrap px-4 py-3">
                          <div className="flex items-center gap-2">
                            <button
