...
const OPS_MODULE_COPY: Record<string, { title: string; desc: string }> = {
  ...
  tailgate: { title: 'Tailgate Logs', desc: 'Video logs, unidentified entries and violation tracking for this centre.' },
...
const OpsModulePlaceholder: React.FC<{ module: string; centreName?: string }> = ({ module, centreName }) => {
  ...
  return (
    <div className="cmx-placeholder">
      ...
      {centreName && (
        <div style={{ marginTop: 4, fontSize: 12, color: 'var(--sub)' }}>Centre: {centreName}</div>
      )}
      <div
                ...
                return (
                  <div key={m.id ?? `${m.code}-${i}`} className="cmx-tbl-wrap" style={{ padding: 16 }}>
                    ...
                    <details style={{ marginTop: 12 }}>
                      <summary
                        style={{ cursor: 'pointer', fontSize: 12, fontWeight: 600, color: 'var(--blue)' }}
                      >
                        Full configuration
                      </summary>
                      <div style={{ marginTop: 10 }}>
