...
const UserList: React.FC = () => {
  ...
  const userColumns: ColumnDef[] = [
    ...
    },
    {
      ...
      flex: 1.5,
      renderCell: (params) => (
        <span className={`inline-flex items-center px-3 py-1.5 rounded-lg text-[10px] font-semibold border transition-all ${
          params.value === 'Admin'
    ...
    <div className="max-full">
      ...
      <div className={`grid grid-cols-1 ${selectedUser ? 'lg:grid-cols-[1fr_350px]' : 'w-full'} gap-6 `}>
        <UserTable
          ...
          emptyState={{
            icon: <svg className="w-16 h-16 text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 h-fit sticky top-8">
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <span className="text-sm text-gray-800 font-medium">{selectedUser.id}</span>
              </div>
              <div className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                  Name:
                </span>
                <span className="text-sm text-gray-800 font-medium">{selectedUser.name}</span>
              </div>
              <div className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                  Email:
                </span>
                <span className="text-sm text-gray-800 font-medium">{selectedUser.email}</span>
              </div>
              <div className="flex flex-col gap-1.5">
                <span className="text-xs font-semibold text-gray-600 uppercase tracking-wide">
                  Role:
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <span className="text-sm text-gray-800 font-medium">
                  <span
                    className={`inline-block px-3 py-1 rounded-full text-xs font-medium capitalize ${selectedUser.status === 'active'
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <span className="text-sm text-gray-800 font-medium">{selectedUser.createdAt}</span>
              </div>
            </div>
          </div>
        )}
